...
pub fn part_one(input: &str) -> Option<usize> {
    ...
    Some(
        drive
            .iter()
            .enumerate()
            .map(|(i, x)| if x.is_some() { x.unwrap() * i } else { 0 })
            .sum(),
    )
}
...
pub fn part_two(input: &str) -> Option<usize> {
    ...
    loop {
        ...
        last = last.saturating_sub(file_size + 1);
    }
